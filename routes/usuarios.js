...
router.route("/actualizar_usuario")
    .put([checkToken, verificarRolAdministrador], async (req, res) => {
        let {usuario, nombre, email, telefono, direccion, contrasena, es_admin, oldUser} = req.body;
        ...
        if (usuario_db !== undefined && usuario_db.length > 0) {
            ...
            await Usuario.actualizar(usuario, nombre, email,
                telefono, direccion, contrasena, Number.isInteger(es_admin), oldUser);
...
router.route("/crear_usuario")
    .post([checkToken, verificarRolAdministrador],async (req, res) => {
        const {usuario, nombre, email, telefono, direccion, contrasena, es_admin} = req.body;
        const data = {usuario, nombre, email, telefono, direccion};
        const usuarios_db = await Usuario.buscarXUsuario(usuario);
        if (usuarios_db !== undefined && usuarios_db.length === 0) {
            await Usuario.crear(usuario, nombre, email, telefono, direccion, contrasena, es_admin);
            const token = tokenManejador.crearToken(data);
